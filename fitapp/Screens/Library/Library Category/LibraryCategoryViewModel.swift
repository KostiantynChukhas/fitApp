//
//  LibraryCategoryViewModel.swift
//  fitapp
//
//  on 11.05.2023.
//
...
class LibraryCategoryViewModel: DeinitAnnouncerType {
    // MARK: - Private Properties
    
    private let itemListObservable = BehaviorRelay<[TagCategoryModel]>(value: [])

    fileprivate let coordinator: LibraryCategoryCoordinator

    init(_ coordinator: LibraryCategoryCoordinator) {
        self.coordinator = coordinator
        setupSectionsObservable()
    }
    
    func route(to route: LibraryCategoryCoordinator.Route) {
        coordinator.route(to: route)
    }
}

extension LibraryCategoryViewModel: ViewModelProtocol {
    struct Input {
    ...
    func transform(_ input: Input, outputHandler: @escaping (Output) -> Void) {
        input.disposeBag.insert([
            ...
            setupApplyButtonObservable(with: input.applySignal),
    ...
    private func setupSectionsObservable() {
        ...
        let selectedCategories = LibraryCategoryService.shared.getCategories()
    ...
    private func setupModelSelectedObservable(with signal: Observable<TagCategoryModel>) -> Disposable {
        signal.bind { [weak self] model in
            ...
            LibraryCategoryService.shared.addCategory(categories: selectedCategories)
        }
    }
    ...
    private func setupDefaultButtonObservable(with signal: Observable<Void>) -> Disposable  {
        signal.bind { [weak self] in
            ...
            LibraryCategoryService.shared.addCategory(categories: items)
        }
    }
    
    private func setupApplyButtonObservable(with signal: Observable<Void>) -> Disposable  {
        signal.bind { [weak self] in
            self?.route(to: .back)
        }
    }
