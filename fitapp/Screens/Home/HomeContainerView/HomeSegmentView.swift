...
class HomeSegmentView: FitView {
    internal struct Signals {
        fileprivate let didEndScrollToItem: Publisher<Int> = .init()
        fileprivate let homeSegmentUpdated: Publisher<Void> = .init()
    }
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.registerCellClass(HomeSegmentCollectionViewCell.self)
    ...
    func scrollTo(index: Int) {
        guard countDataSource.value >= 3 else { return }
        collectionView.scrollToItem(at: .init(row: index), at: .centeredHorizontally, animated: true)
    }
}

extension HomeSegmentView: NonReusableViewProtocol {
    
    func setupOutput(_ viewModel: HomeSegmentViewModel) {
        ...
        let input = HomeSegmentViewModel.Input(willDisplayScreenByIndex: willDisplayScreenByIndex,
                                               ...
                                               homeSegmentUpdated: signals.homeSegmentUpdated.asObservable(),
                                               disposeBag: disposeBag)
        
        viewModel.transform(input, outputHandler: setupInput(input:))
    }
    
    private func setupInput(input: HomeSegmentViewModel.Output) {
        ...
        disposeBag.insert([
            ...
            setupHomeSegmentUpdated(with: input.dataSource.asObservable().mapToVoid()),
            setupIndexPathSelected(with: indexPathSelected)
        ])
    }
    
    private func setupHomeSegmentUpdated(with signal: Observable<Void>) -> Disposable {
        signal.delay(.milliseconds(200), scheduler: MainScheduler.instance).bind(to: signals.homeSegmentUpdated)
    }
    ...
    private func setupDataSourceObserving(with signal: Driver<[UIViewController]>) -> Disposable {
        signal.drive(collectionView.rx.items) { collectionView, row, vc in
            let cell = collectionView.dequeueReusableCell(ofType: HomeSegmentCollectionViewCell.self, at: .init(row: row))
