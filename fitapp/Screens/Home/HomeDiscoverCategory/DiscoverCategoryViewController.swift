//
//  HomeDiscoverCategoryViewController.swift
//  fitapp
//
...
class DiscoverCategoryViewController: ViewController<DiscoverCategoryViewModel> {
    ...
    override func setupOutput() {
        let input = DiscoverCategoryViewModel.Input(
    ...
    override func setupInput(input: DiscoverCategoryViewModel.Output) {
        super.setupInput(input: input)
        
        disposeBag.insert(
            setupDiscoverListObserving(with: input.tableItemsObservable)
        )
    }
    ...
    private func setupDiscoverListObserving(with signal: Driver<[TagCategoryModel]>) -> Disposable {
        signal
