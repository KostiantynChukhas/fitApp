 ...
 */
internal func react<State, Request: Equatable, Event>(
 ...
 * In case new elements are present in `new` request (and not in `old` request) they are being passed to the `effects` lambda and resulting effects are being performed.
 ...
 - returns: The feedback loop performing the effects.
 */
internal func react<State, Request, Event>(
...
private class RequestLifetimeTracking<Request: Equatable, RequestID: Hashable, Event> {
    class LifetimeToken {}
...
/**
 ...
 - returns: The feedback loop performing the effects.
 */
internal func react<State, Request: Equatable, RequestID, Event>(
 ...
 */
internal class Bindings<Event>: Disposable {
     ...
     */
    internal init(subscriptions: [Disposable], events: [Observable<Event>]) {
    ...
    /**
     - parameters:
        ...
        - events: mappings of events from UI to events of a given system
     */
    internal init(subscriptions: [Disposable], events: [Signal<Event>]) {
        self.subscriptions = subscriptions
 ...
 */
internal func bind<State, Event>(_ bindings: @escaping (ObservableSchedulerContext<State>) -> (Bindings<Event>)) -> (ObservableSchedulerContext<State>) -> Observable<Event> {
...
/**
 ...
 Strongify owner.
 */
internal func bind<State, Event, WeakOwner>(_ owner: WeakOwner, _ bindings: @escaping (WeakOwner, ObservableSchedulerContext<State>) -> (Bindings<Event>))
...
/**
 Bi-directional binding of a system State to external state machine and events from it.
 */
internal func bind<State, Event>(_ bindings: @escaping (Driver<State>) -> (Bindings<Event>)) -> (Driver<State>) -> Signal<Event> {
    return { (state: Driver<State>) -> Signal<Event> in
        Observable<Event>.using(
            { () -> Bindings<Event> in
                bindings(state)
...
/**
 ...
 Strongify owner.
 */
internal func bind<State, Event, WeakOwner>(_ owner: WeakOwner, _ bindings: @escaping (WeakOwner, Driver<State>) -> (Bindings<Event>))
    ...
    -> (O) -> (Bindings<Event>) where WeakOwner: AnyObject
{
    return { [weak owner] state -> Bindings<Event> in
        guard let strongOwner = owner else {
            ...
            return Bindings(subscriptions: [], events: observable)
        }
