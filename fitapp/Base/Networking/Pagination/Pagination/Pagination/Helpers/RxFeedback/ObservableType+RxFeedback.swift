...
extension ObservableType where Element == Any {
    /// Feedback loop
    internal typealias Feedback<State, Event> = (ObservableSchedulerContext<State>) -> Observable<Event>
    internal typealias FeedbackLoop = Feedback
     ...
     */
    internal static func system<State, Event>(
        initialState: State,
...
extension SharedSequenceConvertibleType where Element == Any, SharingStrategy == DriverSharingStrategy {
    /// Feedback loop
    internal typealias Feedback<State, Event> = (Driver<State>) -> Signal<Event>
...
internal struct ObservableSchedulerContext<Element>: ObservableType {
    internal typealias E = Element

    /// Source observable sequence
    internal let source: Observable<Element>

    /// Scheduler on which observable sequence receives elements
    internal let scheduler: ImmediateSchedulerType
    ...
    internal init(source: Observable<Element>, scheduler: ImmediateSchedulerType) {
        self.source = source
        self.scheduler = scheduler
    }

    internal func subscribe<Observer: ObserverType>(_ observer: Observer) -> Disposable where Observer.Element == Element {
